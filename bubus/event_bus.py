...
class EventBus:
    ...
    async def process_event(self, event: 'BaseEvent[Any]', timeout: float | None = None) -> None:
        ...
        # Create pending results using the new BaseEvent method
        event.create_pending_results(applicable_handlers)
        ...
        event.check_if_all_handlers_completed()
        ...
        while current.event_parent_id and current.event_parent_id not in checked_ids:
            ...
            if parent_event.event_completed_signal and not parent_event.event_completed_signal.is_set():
                parent_event.check_if_all_handlers_completed()
    ...
    async def _execute_handlers(
        ...
        if not applicable_handlers:
            event.check_if_all_handlers_completed()  # mark event completed immediately if it has no handlers
            return
